...
const PAGES = [
  {
    ...
    coverageP:
      'Coast to coast or two states over, the promise is the same: the long distance movers who load your house are the ones who unload it, and the delivery date is the one written into your contract.',
    servicesH2: 'Long distance movers, <em>at your level of help.</em>',
    servicesP:
      'Distance changes a move: everything has to survive days on the road, and nothing can be driven back for. Pick the level of help you want, and our long distance movers plan the load around the miles it has to cover.',
    processH2: 'How a long-haul move <em>actually</em> runs.',
    processP:
      'Survey, binding quote, load, delivery. Four steps across the whole country, run by long distance movers who price the entire distance up front rather than revising it on delivery day.',
    processSteps: [
    ...
    jobLede: 'Photographs from our own long distance movers — this is what the four parts of a cross-country load look like.',
    jobSteps: [
    ...
    galleryP: 'Homes and offices our long distance movers have carried across the country — packed, loaded and delivered by the crews in these photographs.',
    reviewsH2: 'Why people choose these long distance <em>movers.</em>',
    faqH2: 'Long distance movers, <em>questions answered.</em>',
    faqP: 'What people ask before booking long distance movers. If yours is not here, call — we are around.',
    faq: [
      ...
      ['How do I choose between long distance movers?',
       '<p>Ask three things. Does the company hold its own FMCSA authority, or is it a broker reselling your job? Is the estimate binding and in writing after a survey, or a number over the phone? And are the long distance movers at your door employees of the company you signed with? We answer yes, yes and yes — and will put all three in writing before you book.</p>'],
      ['Do you use your own trucks and crews?',
       '<p>Yes. The truck at your door is ours, and the long distance movers loading it are our employees, at both ends of the move. Nothing is auctioned to a third party after you book, which is the single most common way a cross-country move goes wrong.</p>'],
  ...
  },
  {
    ...
    coverageP:
      'Cross a state line and your move stops being a local job and becomes a federally regulated one. We are licensed interstate movers for all of it — every continental state, under our own USDOT authority.',
    servicesH2: 'Interstate movers, <em>done by the book.</em>',
    servicesP:
      'Pick the level of help you want. Whichever you choose, the paperwork underneath is the same, because interstate movers are federally regulated: a binding written estimate, a signed inventory, and valuation coverage set before we load.',
    processH2: 'How an interstate move <em>actually</em> works.',
    processP:
      'Survey, binding estimate, load, delivery — with every document licensed interstate movers owe you handed over at the step it belongs to, not after the fact.',
    processSteps: [
    ...
    jobLede: 'Photographs from our own interstate movers — this is what the four parts of a load across the state line look like.',
    jobSteps: [
    ...
    galleryP: 'Homes and offices our interstate movers have carried over state lines — packed, loaded and delivered by the crews in these photographs.',
    reviewsH2: 'Why people choose these interstate <em>movers.</em>',
    faqH2: 'Interstate movers, <em>questions answered.</em>',
    faqP: 'The questions that come up when you are hiring interstate movers. If yours is not here, call — we are around.',
    faq: [
      ['How do I check that interstate movers are legitimate?',
       '<p>Every interstate mover must hold a USDOT number and FMCSA operating authority for household goods, and you can look both up on the FMCSA register before you sign anything. Ours are on every page of this site: USDOT #4575745, MC-1820728. Interstate movers who will not give you those numbers are telling you something.</p>'],
      ['Are you licensed interstate movers?',
  ...
  },
  {
    ...
    coverageP:
      'Pick your old state and your new one: as state to state movers we run between all 48 and D.C., and we have a guide for each of them — what a move out of there costs you in time, season and paperwork.',
    servicesH2: 'State to state movers, <em>at your pace.</em>',
    servicesP:
      'A move between states is really two local moves with a long drive in the middle. Choose how much of it you want to hand over — the same state to state movers handle both ends either way.',
    processH2: 'What moving between states <em>looks like.</em>',
    processP:
      'One survey, one price, one crew, both addresses: the state to state movers who load you are the ones who deliver you. Nothing is handed to a partner company the moment you cross the line.',
    processSteps: [
    ...
    jobLede: 'Photographs from our own state to state movers — this is what moving between two states looks like from the inside.',
    jobSteps: [
    ...
    galleryP: 'Moves our state to state movers have run this year — packed, loaded and delivered by the crews in these photographs.',
    reviewsH2: 'Why people choose these state to state <em>movers.</em>',
    faqH2: 'State to state movers, <em>questions answered.</em>',
    faqP: 'What people ask when they are hiring state to state movers. If yours is not here, call — we are around.',
    faq: [
      ['What should I look for in state to state movers?',
       '<p>One company on both sides of the line, a binding written price after a real survey, and its own FMCSA authority rather than a broker\'s. State to state movers who pass your shipment to a partner firm at the border are the ones people write the bad reviews about — that handover is where responsibility goes missing.</p>'],
